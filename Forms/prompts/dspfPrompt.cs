...
{
    public partial class dspfPrompt : Form
    {
        public dspfPrompt()
        {
            InitializeComponent();
        ...
        private string[] generateCommands()
        {
            ...
            list.Add("CPYFRMSTMF FROMSTMF('" + textBox2.Text + "') TOMBR('/QSYS.lib/QTEMP.lib/IBMICMDSRC.file/THEDSPF.mbr') MBROPT(*ADD)");
            list.Add("CRTDSPF FILE(" + textBox1.Text.ToUpper() + ") SRCFILE(QTEMP/IBMICMDSRC) SRCMBR(THEDSPF) " + textBox4.Text);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (validate())
            {
                IBMi.setConfig("DSPFBind", string.Join("|", generateCommands()));
                bindings = IBMi.getConfig("binds").Split('|').ToList();
                bindings.Add("DSPFBind");
